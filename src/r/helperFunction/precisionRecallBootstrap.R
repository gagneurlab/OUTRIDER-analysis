...
bootstrapPR <- function(score, label, total=sum(label), decr=TRUE, n=500, mc.cores=10){
    ...
    d <- rbindlist(mclapply(1:n, mc.cores=mc.cores, function(i){
    ...
    td <- calculatePrecisionRecall(score=score, label=label, decr=decr, total=total)
    
    d[,tPrecision:=td$precision]
    
    # estimating the quantiles (just taking the nth element)
    d <- d[order(rank, precision - tPrecision)]
    lowerDiff <- d[, .(q=(precision - tPrecision)[max(1, floor(length(precision)*0.025))]), by=rank]
    upperDiff <- d[, .(q=(precision - tPrecision)[ceiling(length(precision)*0.975)]), by=rank]
    td[, lower:=precision + lowerDiff$q]
    td[, upper:=precision + upperDiff$q]
